...
async function main() {
  ...
  await prisma.storeSettings.create({
    data: {
      ...
      storeDescription: 'E-commerce de camisas de futebol retro e oficiais. Qualidade premium, preco justo.',
      mainProducts: 'Camisas de futebol retro, camisas oficiais, acessorios esportivos',
      targetAudience: 'Torcedores apaixonados, colecionadores de camisas, homens 18-45 anos',
      shippingPolicy: 'Frete gratis acima de R$299. Envio em ate 3 dias uteis. Entrega para todo Brasil.',
      returnPolicy: 'Troca gratis em ate 30 dias. Devolvemos o dinheiro se preferir.',
      paymentMethods: 'PIX (5% desconto), cartao de credito ate 6x sem juros, boleto bancario',
      warrantyPolicy: 'Garantia de 90 dias contra defeitos de fabricacao',
      faqContent: 'Tabela de medidas disponivel em cada produto.',
      ...
      aiTone: 'amigavel',
      aiName: 'Ana',
      customInstructions: 'Sempre mencionar a qualidade das camisas e a paixao pelo futebol. Usar linguagem de torcedor.',
  ...
  await prisma.storeSettings.create({
    data: {
      ...
      mainProducts: 'Camisas oficiais, tenis esportivos, shorts, meias, acessorios',
      targetAudience: 'Torcedores de todos os clubes, praticantes de esportes',
      shippingPolicy: 'Frete gratis acima de R$199. Envio em ate 5 dias uteis.',
  ...
  for (const data of cartData) {
    ...
    const cart = await prisma.abandonedCart.create({
      data: {
        ...
        type: data.type,
        status: data.status,
        recoveryAttempts: data.status === 'CONTACTING' ? 1 : data.status === 'RECOVERED' || data.status === 'PAID' ? 2 : 0,
        abandonedAt: new Date(now.getTime() - data.hoursAgo * 3600000),
        recoveredAt: data.status === 'RECOVERED' || data.status === 'PAID' ? new Date(now.getTime() - (data.hoursAgo - 1) * 3600000) : null,
        recoveredValue: data.status === 'RECOVERED' || data.status === 'PAID' ? data.total : null,
  ...
  for (const idx of contactingIndices) {
    ...
    const conv = await prisma.conversation.create({
      data: {
        ...
        status: cartInfo.status === 'RECOVERED' || cartInfo.status === 'PAID' ? 'RECOVERED' : 'ACTIVE',
        aiModel: 'gpt-4o-mini',
      ...
      data: [
        {
          ...
          role: 'AI',
          content: `Ola ${cart.customerName?.split(' ')[0]}! Vi que voce estava dando uma olhada nos nossos produtos. Posso te ajudar a finalizar a compra?`,
        ...
        },
        {
          conversationId: conv.id,
          role: 'CUSTOMER',
          content: 'Oi! Sim, eu estava olhando mas achei o frete caro...',
          messageStatus: 'READ',
          sentAt: new Date(now.getTime() - 3500000),
        },
        {
          conversationId: conv.id,
          role: 'AI',
          content: 'Entendo! Sabia que acima de R$299 o frete e gratis? E tenho um cupom especial de 10% pra voce: VOLTA10',
          messageStatus: 'DELIVERED',
          sentAt: new Date(now.getTime() - 3400000),
        },
      ],
    })
  }

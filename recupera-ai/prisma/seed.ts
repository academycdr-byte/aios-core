...
async function main() {
  ...
  // Create single store (1 user = 1 store)
  ...
  await prisma.storeSettings.create({
    data: {
      ...
      shippingPolicy: 'Frete grátis acima de R$299. Envio em até 3 dias úteis.',
      returnPolicy: 'Troca grátis em até 30 dias.',
      paymentMethods: 'PIX (5% desconto), cartão até 6x sem juros, boleto',
      ...
      aiTone: 'amigavel',
      aiName: 'Ana',
      customInstructions: 'Mencionar qualidade e paixão pelo futebol.',
  ...
  await prisma.recoveryConfig.create({
    data: { storeId: store.id, isActive: true, firstMessageDelay: 30, followUp1Delay: 360, followUp2Delay: 1440, maxAttempts: 3, minCartValue: 50, pixRecoveryEnabled: true, pixFirstDelay: 15, pixFollowUpDelay: 120, pixMaxAttempts: 2, cardRecoveryEnabled: true, cardFirstDelay: 10, cardMaxAttempts: 2 },
  })
  console.log('Recovery config created')

  // Create sample abandoned carts (all in the single store)
  const now = new Date()
  const cartData = [
    { name: 'João Silva', email: 'joao@email.com', phone: '5511999887766', total: 389.90, items: [{ name: 'Camisa Flamengo Retro 1981', qty: 1, price: 249.90 }, { name: 'Shorts Casual', qty: 1, price: 140.00 }], type: 'ABANDONED_CART', status: 'PENDING', hoursAgo: 1 },
    ...
    { name: 'Carlos Mendes', email: 'carlos@email.com', phone: '5511955443322', total: 149.90, items: [{ name: 'Meião Oficial', qty: 3, price: 49.97 }], type: 'CARD_DECLINED', status: 'PENDING', hoursAgo: 2 },
    { name: 'Lúcia Ferreira', email: 'lucia@email.com', phone: '5521944332211', total: 679.90, items: [{ name: 'Camisa Vasco Retro', qty: 1, price: 289.90 }, { name: 'Camisa São Paulo', qty: 1, price: 390.00 }], type: 'ABANDONED_CART', status: 'CONTACTING', hoursAgo: 6 },
    { name: 'Ricardo Alves', email: 'ricardo@email.com', phone: '5531933221100', total: 299.90, items: [{ name: 'Tênis Nike Football', qty: 1, price: 299.90 }], type: 'PIX_PENDING', status: 'PAID', hoursAgo: 48 },
    { name: 'Fernanda Costa', email: 'fernanda@email.com', phone: '5541922110099', total: 189.90, items: [{ name: 'Camisa Grêmio Away', qty: 1, price: 189.90 }], type: 'ABANDONED_CART', status: 'LOST', hoursAgo: 72 },
  ]
  ...
  for (const idx of contactingIndices) {
    ...
    await prisma.message.createMany({
      data: [
        { conversationId: conv.id, role: 'AI', content: `Olá ${cart.customerName?.split(' ')[0]}! Vi que você estava olhando nossos produtos. Posso te ajudar?`, messageStatus: 'DELIVERED', sentAt: new Date(now.getTime() - 3600000) },
        ...
        { conversationId: conv.id, role: 'AI', content: 'Entendo! Acima de R$299 o frete é grátis! E tenho um cupom de 10% pra você: VOLTA10', messageStatus: 'DELIVERED', sentAt: new Date(now.getTime() - 3400000) },
      ],
    })
  }
  console.log('Conversations and messages created')

  // Create daily metrics (last 30 days — single store)
  for (let d = 29; d >= 0; d--) {
